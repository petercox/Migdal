...
dpI1E::usage="Differential probability for single ionisation with excitation.";
...
dpI1dipole::usage="Differential dipole probability"
pI1dipole::usage"Integrated dipole probability."
	...
	,
		...
		,
			If[OptionValue["EThreshold"] === None,
				...
				,
				Return[If[#1>Log[EThreshold], (Exp[#2]/v0)^2 Exp[diff[#1]]&, 0]&];
			];
		];
	,
		(* Single ionisation *)
		...
		,
			...
			If[OptionValue["EThreshold"] === None,
				Return[Exp[diff[##]]&];
				,
				Return[If[#1>Log[EThreshold], Exp[diff[##]], 0]&];
			];
		];
	,
		(* Double ionisation *)
		...
		,
			...
			If[OptionValue["EThreshold"] === None,
				Return[{0&, Exp[diff2[##]]&}];
				,
				Return[{If[#1>Log[EThreshold],2*NIntegrate[Exp[lnEn2] Exp[diff2[#1,lnEn2,#2]],{lnEn2,Log[$emin],Log[EThreshold]},Method->{"InterpolationPointsSubdivision",Method->"LocalAdaptive"},PrecisionGoal->OptionValue["PrecisionGoal"]],0]&, If[#1>Log[EThreshold] && #2>Log[EThreshold], Exp[diff2[##]], 0]&}];
			];
		];
	];
];
...
Options[LoadTotalProbabilities]=Flatten[{Options[MigdalInterpolate],{"DarkMatter"->False, "Dipole"->False, "Double"->True, "Excitations"->False, "Inclusive"->False, "VelocityGrid"->"linear"}}];
SyntaxInformation[LoadTotalProbabilities] = {"ArgumentsPattern" -> {_, OptionsPattern[]}};

LoadTotalProbabilities[element_,opts:OptionsPattern[]]:= Module[{ppI1,ppI1E,ppI2},
	...
	If[OptionValue["Excitations"],
		...
		(* Single excitation *)
		pIE = MigdalInterpolate[element<>"/excitations/"<>element<>"_single-excitation.txt",-1];
		pE = pIE;
		...
		If[OptionValue["Double"],
			...
			ppI1E = MigdalInterpolate[element<>"/excitations/"<>element<>"_excitation+ionisation.txt",1,FilterRules[{opts}, Options[MigdalInterpolate]]];
			If[OptionValue["Integrated"], pI1E = ppI1E, dpI1E = ppI1E];
		];
	];
	...
	(* Total probabilities *)
	If[OptionValue["Integrated"], ptotal[lnv_]:= p0[lnv] + pE[lnv] + pI1[lnv] + pI1E[lnv] + pI2[lnv];];
...
Options[LoadProbabilities]=Flatten[{Options[MigdalInterpolate],{"DarkMatter"->False, "Dipole"->False, "Double"->True, "Inclusive"->False, "VelocityGrid"->"linear"}}];
...
LoadProbabilities[element_,opts:OptionsPattern[]]:=Module[{i, j, pair, ppI1, ppI2},
	MigdalResetOrbitals[Dipole->OptionValue["Dipole"], Integrated->OptionValue["Integrated"]];
	...
	(* Dipole single ionisation *)
	If[OptionValue["Dipole"],
		ppI1 = Association[#[[1]]->MigdalInterpolate[element<>"/single-ionisation/"<>element<>"_single-ionisation_dipole_"<>#[[1]]<>".txt",11,FilterRules[{opts}, Options[MigdalInterpolate]]]& /@ $orbitals[[element]]];
		If[OptionValue["Integrated"], 
			pI1dipole = ppI1;
...
Options[MigdalReset]= {"Integrated"->False};
...
MigdalReset[OptionsPattern[]]:=Module[{},
	...
	If[OptionValue["Integrated"],
		Clear[pI1,pI1E,pI21,pI2,ptotal];
		pI1[lnv_]:= 0;
		pI1E[lnv_]:= 0;
		pI21[lnv_]:= 0;
		pI2[lnv_]:= 0;
...
MigdalResetOrbitals::usage="MigdalReset[]
Set probabilities to zero."
Options[MigdalResetOrbitals]= {"Dipole"->False, "Integrated"->False};
SyntaxInformation[MigdalResetOrbitals] = {"ArgumentsPattern" -> {OptionsPattern[]}};

MigdalResetOrbitals[OptionsPattern[]]:=Module[{},
